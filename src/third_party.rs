...
pub fn vt_fetch(key: &str, hash: &str) -> HashMap<String, String> {
    ...
    let response = client
        ...
        .send().unwrap()
        .text().unwrap();
    let mut response_json = serde_json::from_str::<serde_json::Value>(&response).unwrap();
    if let Some(attributes) = response_json.get("data").and_then(|d| d.get("attributes")){
        ...
        let mut results: HashMap<String, String> = HashMap::new();
       results.insert("No results".to_string(), "No virustotal results".to_string());
       results
    }
